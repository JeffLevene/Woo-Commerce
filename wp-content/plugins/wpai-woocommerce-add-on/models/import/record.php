...
class PMWI_Import_Record extends PMWI_Model_Record {		
	...
	public function parse($parsing_data = array()) { //$import, $count, $xml, $logger = NULL, $chunk = false, $xpath_prefix = ""
		...
		if ($import->options['is_multiple_grouping_product'] != 'yes' and "" != $import->options['single_grouping_product']){
	...
	public function import($importData = array()){ //$pid, $i, $import, $articleData, $xml, $is_cron = false, $xpath_prefix = ""
		...
		// VARIATIONS
		if ( $product_type == 'variable' and ! $import->options['link_all_variations'] and "xml" != $import->options['matching_parent'] ) {
			...
			if (!empty($product_parent_post_id) and ($product_parent_post_id != $pid or ($product_parent_post_id == $pid and $first_is_parent == "no"))) {
				...
				if ( $import->options['update_all_data'] == "yes" or ( $import->options['update_all_data'] == "no" and $import->options['is_update_attributes']) or empty($articleData['ID'])){
					foreach ($serialized_attributes as $attr_name => $attr_data) {
						...
						if ( $import->options['update_all_data'] == "no" and $import->options['update_attributes_logic'] == 'only'){
							if ( ! empty($import->options['attributes_list']) and is_array($import->options['attributes_list'])){
								if ( ! in_array( ( (intval($attr_data['in_taxonomy'][$i])) ? "pa_" . $attr_name : $attr_name ) , array_filter($import->options['attributes_list'], 'trim'))) continue;
							}
							else break;								
						}	
						...
						if ( $import->options['update_all_data'] == "no" and $import->options['update_attributes_logic'] == 'all_except'){
							if ( ! empty($import->options['attributes_list']) and is_array($import->options['attributes_list'])) {
								if ( in_array( ( ($is_taxonomy) ? "pa_" . $attr_name : $attr_name ) , array_filter($import->options['attributes_list'], 'trim'))) continue;									
							}
						}						
					 		...
					 		if ( ! is_wp_error($terms) ){					 								 					 							 			
								if ( count($terms) > 0 ){	
								    foreach ( $terms as $term ) {									    										    	
								    	...
								    	if ( strtolower($term->name) == trim(strtolower($values)) ) {
				...
				if ( $import->options['update_all_data'] == "yes" or ( $import->options['update_all_data'] == "no" and $import->options['is_update_attributes'] ) or empty($articleData['ID']) ){
					...
					foreach ( $attributes as $attribute ) {															
						...
						foreach ( $children as $child ) {
							...
							$value = array_map( 'stripslashes', array_map( 'strip_tags',  explode("|", trim( get_post_meta($child, 'attribute_'.$attribute['name'], true)))));
							...
							if ( $attribute['is_variation'] ) {							
								...
								if (!empty($value) and empty($default_attributes[ $attribute['name'] ]))
									$default_attributes[ $attribute['name'] ] = sanitize_title((is_array($value)) ? $value[0] : $value);
							...
							if ( ! empty($values) ) {				 												
						 		...
						 		if ( ! is_wp_error($terms) ){
							 		...
							 		foreach ($values as $key => $value) {
										...
										if ( count($terms) > 0 ){	
										    foreach ( $terms as $term ) {										    	
										    	if ( strtolower($term->name) == trim(strtolower($value)) or strtolower($term->slug) == trim(strtolower($value)) ) {
										    		$attr_values[] = $term->slug;
										    		$term_founded = true;
										    		break;
										    	}
			...
			if (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, '_regular_price')) update_post_meta( $pid, '_regular_price', '' );
			if (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, '_sale_price')) update_post_meta( $pid, '_sale_price', '' );
			if ( class_exists('woocommerce_wholesale_pricing') and (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, 'wholesale_price'))) update_post_meta( $pid, 'pmxi_wholesale_price', '' );
			if (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, '_sale_price_dates_from')) update_post_meta( $pid, '_sale_price_dates_from', '' );
			if (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, '_sale_price_dates_to')) update_post_meta( $pid, '_sale_price_dates_to', '' );
			if (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, '_price')) update_post_meta( $pid, '_price', '' );			
			...
			if ( $count_variations > 0 ):
				...
				if (!empty($import->options['variable_whosale_price'])){
					if ($import->options['variable_whosale_price_use_parent']){
						...
						if (!empty($parent_whosale_price[0])){
							count($variation_sku) and $variation_whosale_price = array_fill(0, count($variation_sku), $parent_whosale_price[0]);
						}
						else{
				...
				foreach ($variation_sku as $j => $void) {	if ("" == $variation_sku[$j]) continue;
					...
					// Manage stock
					if (empty($articleData['ID']) or $this->is_update_custom_field($existing_variation_meta_keys, $import->options, '_stock')) update_post_meta( $variation_to_update_id, '_stock', (!empty($variation_stock[$j])) ? $variation_stock[$j] : 0 );
